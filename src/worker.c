...
void execute_solution(char *executable_path, int param, int batch_idx) {
    ...
    if (pid == 0) {
        ...
        execlp(executable_path, executable_name, param, (char *) NULL);
...
void monitor_and_evaluate_solutions(int finished) {
    ...
    for (int j = 0; j < curr_batch_size; j++) {
        ...
        write(STDERR_FILENO, current_exe_path, strlen(current_exe_path));
        write(STDERR_FILENO, "here\n", 5);

        int status;
        pid_t pid = waitpid(pids[j], &status, 0);
        // probably hanging here

        while (pid == -1 && errno == EINTR) {
            pid = waitpid(pids[j], &status, 0);
        }

        write(STDERR_FILENO, "child ret\n", 10);
        ...
        if (signaled) {
            ...
            if (signal_number == SIGKILL) {
                ...
                // pairs[finished - curr_batch_size + j].status = STUCK_OR_INFINITE;
                pairs[finished + j].status = STUCK_OR_INFINITE;
            } else if (signal_number == SIGSEGV) {
                // Child process triggered a segmentation fault
                // pairs[finished - curr_batch_size + j].status = SEGFAULT;
                pairs[finished + j].status = SEGFAULT;
            }
            write(STDERR_FILENO, "status set\n", 11);
        }

        // TODO: Also, update the results struct with the status of the child process
        char output_file[BUFSIZ];
        write(STDERR_FILENO, "trying to access pairs index\n", 29);
        write(STDERR_FILENO, pairs[finished + j].executable_path, strlen(pairs[finished + j].executable_path));
        write(STDERR_FILENO, "here\n", 5);
        
        char* filename = strrchr(pairs[finished - curr_batch_size + j].executable_path, '/');
        ...
        if (num_bytes != 0) {
            ...
            pairs[finished - curr_batch_size + j].status = atoi(buffer);
        }
        close(output_fd);
    ...
    free(child_status);
    write(STDERR_FILENO, "monitor done\n", 13);
}
...
int main(int argc, char **argv) {
    ...
    // char **exec_paths = malloc(sizeof(char *) * pairs_to_test);
    // for (int i = 0; i < pairs_to_test; ++i) {
    //     exec_paths[i] = malloc(128);
    //     memset(&exec_paths[i], 0, 128);
    //     pairs[i].executable_path = exec_paths[i];
    // }
    ...
    write(STDERR_FILENO, "worker received pairs\n", 22);
    ...
    // TODO: properly handle receiving acks
    ...
    // } while (strcmp("ACK", synacks.mtext) == 0);
